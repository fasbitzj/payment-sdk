...
public class UcloudlinkLogger {
    ...
    public static void setNeedEnableLogger(boolean needEnableLogger) {
        UcloudlinkLogger.needEnableLogger = needEnableLogger;
    }
    ...
    private static void _logCommError(Exception e, HttpURLConnection conn, String url,
        ...
        SimpleDateFormat df = new SimpleDateFormat(UcloudlinkConstants.DATE_TIME_FORMAT);
        df.setTimeZone(TimeZone.getTimeZone(UcloudlinkConstants.DATE_TIMEZONE));
        String sdkName = UcloudlinkConstants.SDK_VERSION;
        String urlStr = null;
        String rspCode = "";
        if (conn != null) {
            try {
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logErrorScene(Map<String, Object> rt, UcloudlinkResponse tRsp,
                                     String appSecret) {
        if (!needEnableLogger) {
            return;
        }
    ...
    private static void appendLog(HashMap map, StringBuilder sb) {
        boolean first = true;
